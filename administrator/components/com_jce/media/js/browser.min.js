 ...
 var tinyMCEPopup = {
 	params : {
		...
		"callback" 	: function(selected, data) {
			var win = window.parent, options = BrowserDialog.settings, value = data[0].url;
	...
	editor : {
		windowManager: {
				open: function(args) {
					...
					var div = $('<div class="ui-inline-help ui-modal" />');

					var html = '<div class="ui-modal-dialog ui-modal-dialog-blank">' +
					'	<button class="ui-modal-close ui-close"></button>' +
					'	<div class="ui-modal-header"><h3><strong>' + (args.title || "") + '</strong></h3></div>' +
					'	<iframe src="' + args.url + '" frameborder="0"></iframe>' +
					'</div>';

					$(div).html(html).find('.ui-close').click(function(e) {
							e.preventDefault();
							$(div).removeClass('ui-open').delay(150).remove();
					});

					$(div).appendTo('body').addClass('ui-open').show();
				}
		},
		settings: {
			"language" : jQuery.Plugin.getLanguage(),
			"site_url" : ""
		}
	},
 	...
 	close: function() {
 		var win = window.parent, options = BrowserDialog.settings;
...
jQuery(document).ready(function($) {
	var win = window.parent, element = win.document.getElementById(BrowserDialog.settings.element);
